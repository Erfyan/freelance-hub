            ...
            <form action="{{ route('clients.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4"
                        class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-white focus:border-cyan-400 focus:ring-cyan-400 focus:outline-none transition-colors @error('notes') border-rose-500 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
